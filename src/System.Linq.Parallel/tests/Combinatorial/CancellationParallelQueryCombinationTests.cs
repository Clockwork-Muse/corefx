...
namespace System.Linq.Parallel.Tests
{
    public static partial class ParallelQueryCombinationTests
    {
        ...
        public static void MaxBy_OperationCanceledException_PreCanceled(LabeledOperation source, LabeledOperation operation)
        {
            CancellationTokenSource cs = new CancellationTokenSource();
            cs.Cancel();

            Functions.AssertIsCanceled(cs, () => operation.Item(DefaultStart, DefaultSize, (start, count, ignore) => source.Item(start, count).WithCancellation(cs.Token)).MaxBy(x => x));
        }
